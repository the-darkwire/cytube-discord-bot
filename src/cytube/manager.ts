...
const handleChangeMedia =
  ...
  (data: any) => {
    const message = `Now playing on **${cytubeChannel}**: ${data.title} [${data.duration}]`;
    const subs = subscriptions.getByCytubeChannel(cytubeChannel);
    for (const sub of subs) {
      void sendMessage(message, sub.discordChannelId).catch((err) => {
        console.error(`[cytube] failed to post to ${sub.discordChannelId}:`, err);
      });
    }
  };
